...
export async function startOpenAIRelay(options = {}) {
  ...
  if (!probe.ok) {
    throw new Error("the ApiCred global store is not usable; refusing to start the opencode relay");
  }
...
export function resolveOpencodeExecutable(base = process.env) {
  ...
  // the apicred shadow shims (bin-opencode, ahead of npm in PATH) route
...
export function buildOpencodeLauncherEnv({ token, instanceId, base = {} }) {
  ...
  if (instanceId) env.APICRED_AGENT_INSTANCE = instanceId;
