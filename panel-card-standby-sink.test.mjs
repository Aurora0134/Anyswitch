...
const AGENT_IDS = ["zcode", "claude", "dsh", "pi", "reasonix", "kimi", "opencode", "qoder", "codex"];
...
// 全 9 卡的默认看板 payload：聚合卡已启动（status=running）空闲且从未活动
...
describe("panel.html 端点卡待命下沉排序", () => {
  it("① 生成中在顶、超宽限落待命层、未启动垫底，层内保持基准顺序", () => {
    ...
    assert.deepEqual(t.appends[0], [
      "zcode",
      "claude", "dsh", "reasonix", "kimi", "opencode", "qoder", "codex",
      "pi",
  ...
  it("② 待命 ≤5s 留上组；跨过 5s 边界那一刻才重排 DOM（顺序没变不动 DOM）", () => {
    ...
    assert.deepEqual(t.appends[0], [
      "zcode", "dsh",
      "claude", "pi", "reasonix", "kimi", "opencode", "qoder", "codex",
    ...
    assert.deepEqual(t.appends[1], [
      "zcode",
      "claude", "dsh", "pi", "reasonix", "kimi", "opencode", "qoder", "codex",
  ...
  it("④ claude 会话整行退出后锚点消失，面板记忆让待命计时连续", () => {
    ...
    assert.deepEqual(t.appends[0], [
      "claude",
      "zcode", "dsh", "pi", "reasonix", "kimi", "opencode", "qoder", "codex",
  ...
  it("⑤ 三段分层全序：生成中/宽限在顶，已启动待命居中，未启动垫底", () => {
    ...
    assert.deepEqual(t.appends[0], [
      "zcode", "dsh",
      "claude", "reasonix", "kimi", "opencode", "qoder", "codex",
      "pi",
