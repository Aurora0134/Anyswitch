...
describe("chain runtime introspection endpoint (/api/internal/route-chain-runtime)", () => {
  ...
  it("(tail) 流式整链全灭时末位节点同样计入降级", async () => {
    const { upstreamFetch, calls } = memberRouter({
      "chan-a": () => statusError(503),
    ...
    await withServer(deps, async (port) => {
      ...
      assert.deepEqual(body.endpoints.zcode.lamps, ["red", "red"], "末位节点连续失败两次后必须亮红");
    });
  });

  it("(tail) 非流式整链全灭同样点亮末位红灯", async () => {
    const { upstreamFetch } = memberRouter({
      "chan-a": () => statusError(503),
      "chan-b": () => statusError(503),
    });
    ...
    await withServer(deps, async (port) => {
      ...
      const body = await (await getRuntime(port)).json();
      assert.deepEqual(body.endpoints.zcode.lamps, ["red", "red"]);
    });
  });

  it("(tail) 末位的可重试流故障（空流）也计入降级计数", async () => {
    const { upstreamFetch } = memberRouter({
      "chan-a": () => statusError(503),
      "chan-b": () => ({ ok: true, status: 200, body: sseStream([]) }),
    });
    const deps = createMockDeps({ upstreamFetch, getKeepAliveConfig: NO_RETRY });
    await withServer(deps, async (port) => {
      for (let round = 0; round < 2; round += 1) {
