...
- `pool-routing.mjs` — 号池路由原语（纯函数＋一张进程内存粘性表）：号池把 2–5 个 provider 收在自己的 id 下，请求命中池 id 时按粘性成员分发，成员失败则退避到下一档；降级锁死除连续失败外可另按最近请求的失败占比判定（`getFailureRateGate` 注入，缺省关闭）。无 IO，store 由调用方传入。
- `chain-routing.mjs` — 路由链原语（纯函数＋进程内存链状态表）：虚拟模型 `auto` 按端点配置的链逐跳走，节点失败后退避（一跳本次请求彻底失败即计数，链尾一跳同样计数；连续失败达阈值或失败占比达阈值二者之一即锁死）、每 5 分钟由请求驱动惰性回链首重试、成功即粘回链首，并产出面板链灯所需的节点状态。无 IO，store 由调用方传入。
