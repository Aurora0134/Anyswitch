...
async function api(method, path, body) {
  ...
  function initSettingsView() {
    const keepAliveSlider = $("keepAliveSlider");
    ...
    const settingsSaving = { keepAlive: false, followAgent: false, injectEffort: false, spark: false, keepAliveRetries: false };
    function applyKeepAliveUi(mode) {
      const m = KEEP_ALIVE_MODES.includes(mode) ? mode : "enhanced";
      const copy = KEEP_ALIVE_COPY[m];
      if (keepAliveSlider) {
        keepAliveSlider.dataset.mode = m;
        for (const btn of keepAliveSlider.querySelectorAll(".seg-btn")) {
          btn.setAttribute("aria-checked", btn.dataset.mode === m ? "true" : "false");
          btn.tabIndex = btn.dataset.mode === m ? 0 : -1;
        }
      }
    ...
    async function loadSettingsState() {
      ...
      try {
        ...
        if (!settingsSaving.keepAlive && d.keepAlive) {
    ...
    async function persistKeepAliveMode(next, previous) {
      ...
      settingsLoadGen += 1;
      if (keepAliveSlider) keepAliveSlider.setAttribute("aria-disabled", "true");
      try {
        ...
        if (keepAliveSlider) keepAliveSlider.setAttribute("aria-disabled", "false");
      }
    }
    if (keepAliveSlider) {
      keepAliveSlider.addEventListener("click", (e) => {
        const btn = e.target.closest(".seg-btn");
        if (!btn) return;
        const previous = keepAliveSlider.dataset.mode || "enhanced";
        const next = btn.dataset.mode;
        if (!KEEP_ALIVE_MODES.includes(next) || next === previous) return;
        persistKeepAliveMode(next, previous);
      });
      keepAliveSlider.addEventListener("keydown", (e) => {
        const previous = keepAliveSlider.dataset.mode || "enhanced";
        const i = KEEP_ALIVE_MODES.indexOf(previous);
        let next = previous;
        if (e.key === "ArrowRight" || e.key === "ArrowDown") next = KEEP_ALIVE_MODES[Math.min(1, i + 1)];
        else if (e.key === "ArrowLeft" || e.key === "ArrowUp") next = KEEP_ALIVE_MODES[Math.max(0, i - 1)];
        else if (e.key === "Home") next = "off";
        else if (e.key === "End") next = "enhanced";
        else return;
        e.preventDefault();
        if (next !== previous) persistKeepAliveMode(next, previous);
      });
    }
